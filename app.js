...
//express-session은 미들웨어로
app.use(session({
    secret :'asdjha!@#@#$dd',//암호화를 위한 keygen
    resave:false,//매 요청마다 기존 세션에 변경사항이 없어도 세션 다시 저장 여부
    saveUninitialized:false//uninitialized 세션을 강제로 저장 아무 내용 없는 세션도 저장 원본true
    }))
    
    

let bodyParser = require("body-parser");//요청바디데이터처리
...
app.use(bodyParser.urlencoded({extended:false}));//bodyparser설정 중첩된 객체 표현은 허용X
